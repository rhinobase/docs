...
Valibot v1.2.0 introduces a type-safe `.examples()` action for providing schema examples. This is the recommended way to add examples to your schemas.

<Callout type="info">
  To use the `.examples()` action, ensure you have the following minimum versions installed:
  - `valibot` - ^1.2.0
...
const schema = v.pipe(
  v.string(),
  v.examples(["hello world", "goodbye world"]),
);
```

You can also combine it with other actions like `description` and `metadata`:
...
const userSchema = v.pipe(
  v.object({
    name: v.pipe(
      v.string(),
      v.examples(["John Doe", "Jane Doe"]),
    ),
    email: v.pipe(
      v.string(),
      v.email(),
      v.examples(["john@example.com"]),
    ),
  }),
