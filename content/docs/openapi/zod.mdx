...
`validator("json", schema)` documents a request body as `application/json`.
`validator("form", schema)` uses `multipart/form-data`. Pass a media type as
the fourth argument when the endpoint accepts a different representation.
When a route has JSON and form validators, the generated OpenAPI request body
keeps a separate schema for each media type.
...
Routes that use `validator()` automatically include a `400` response matching
the validation middleware's error shape. To omit it from a generated document,
set `defaultValidationErrorResponse: false` in the options passed to
`openAPIRouteHandler` or `generateSpecs`. You can pass a response object there
to use a project-specific error response instead.

## Passing Reference

For Zod v4, add a `ref` through [metadata](https://zod.dev/json-schema?id=metadata).
The generated schema is added to `components.schemas` and is used through an
OpenAPI `$ref`.
...
For Zod v3, use the `zod-openapi` library. Here is a demo -
...
<Callout title="Note">
Zod v4 uses its built-in JSON Schema conversion and does not require
`zod-openapi`. Zod v3 requires `zod-openapi@4` for reference support.
</Callout>
