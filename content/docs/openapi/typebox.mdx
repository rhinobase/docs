...
const querySchema = Compile(Type.Object({
    name: Type.Optional(Type.String()),
}));

// Validation for response body, eg - `"Hello, John!"`
const responseSchema = Compile(Type.String());
```
...
Code below shows how you might be using Typebox with Hono for validation.

```ts
import { Hono } from "hono";
import { tValidator } from "@hono/typebox-validator";

const app = new Hono();

app.get("/", tValidator("query", querySchema), (c) => {
...
1. Inplace of importing the validation middleware (`tValidator`), from `@hono/typebox-validator`, import it from `hono-openapi`.
...
3. You can uninstall `@hono/typebox-validator` as it is no longer needed.
...
You can use the `options` property (2nd argument) to pass the reference to the schema -
...
```ts
const successResponseSchema = Compile(Type.Object({
  message: Type.String(),
}));

const errorResponseSchema = Compile(Type.Object({
  error: Type.String(),
}));
