...
Cloudflare's native [Rate Limiting API](https://developers.cloudflare.com/workers/runtime-apis/bindings/rate-limit/) is the simplest approach. It uses Cloudflare's built-in rate limiting infrastructure without requiring external stores.
...
export { DurableObjectRateLimiter } from "hono-rate-limiter";
```
